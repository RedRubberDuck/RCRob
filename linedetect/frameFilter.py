...
class FrameLineSimpleFilter:
    ...
    def __init__(self, perspectiveTrans, ClaheClipLimit=3.0, ClahetileGridSize=(46, 46), kernelSize=6):
        self.clahe = cv2.createCLAHE(
            clipLimit=ClaheClipLimit, tileGridSize=ClahetileGridSize)
    ...
    def filter(self, gray):
        ...
        mask = cv2.adaptiveThreshold(
            grayf, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 91, -43.5)
    ...
    def apply2(self, frame):
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        birdviewGray = self.perspectiveTrans.wrapPerspective(gray)
        imgMasked = cv2.adaptiveThreshold(
            birdviewGray, 1, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, -10.5)
    ...
    def apply3(self, frame):
        ...
        mask = cv2.adaptiveThreshold(
            birdviewGray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, -20.5)
...
class TriangleMaksDrawer:
    ...
    def cornersMaskPolygons1(newSize):
        polygons1 = np.array([[[newSize[1]//1.5, newSize[0]], [newSize[1],
                                                               newSize[0]//2.4], [newSize[1], newSize[0]]]], dtype=np.int32)
        polygons2 = np.array([[[newSize[1]//6.225, newSize[0]],
                               [0, newSize[0]//1.942], [0, newSize[0]]]], dtype=np.int32)
